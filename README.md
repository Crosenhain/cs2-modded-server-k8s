# CS2 Modded Server Helm Chart

Reusable Helm chart for deploying a modded Counter-Strike 2 dedicated server on Kubernetes (ArgoCD friendly).

## Features
- **Auto-Update Watcher**: CronJob checks for game updates and restarts the server if empty.
- **Mod Support**: Pre-configured to handle Metamod and custom files.
...
| `persistence.customHostPath` | Path to custom files | `/path/to/local/data-cs2/custom_files/` |
